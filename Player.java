...
public abstract class Player {
	...
	public Player(int _playerId) {
		...
		// Perhaps by quoting the above out it will stop resetting users before
		// loading, so the user can't be reset,
		// also making the playerName equal the player id it stops it from
		// saving unless a savedgame was loaded...
		// tried, first thing i heard was
		// "lmao i got reset for the 3rd time!"...ffs!
		...
		/*
		 * playerLook[0] = 1; playerLook[1] = 5; playerLook[2] = 5;
		 * playerLook[3] = 6; playerLook[4] = 5; playerLook[5] = 10;
		 */
		...
		/*
		 * pHead=3; pTorso=25; pArms=26; pHands=34; pLegs=36; pFeet=42;
		 * pBeard=0;
		 */
	...
	// bit at position npcId is set to 1 incase player is currently in
	// playerList
	...
	protected void appendPlayerAppearance(stream str) {
		...
		// playerProps.writeByte(1 & 1 >> 2); // playerStatusMask - skull,
		// prayers etc alkup 0
		...
		int combatLevel = 0;
		if (ran > attstr) {
			combatLevel = (int) (((getLevelForXP(playerXP[1])) * 0.5)
					+ ((getLevelForXP(playerXP[3])) * 0.4)
					+ ((getLevelForXP(playerXP[5])) * 0.35) + ((getLevelForXP(playerXP[6])) * 0.3));
		} else if (mag > attstr) {
			combatLevel = (int) (((getLevelForXP(playerXP[1])) * 0.5)
					+ ((getLevelForXP(playerXP[3])) * 0.4)
					+ ((getLevelForXP(playerXP[5])) * 0.35) + ((getLevelForXP(playerXP[4])) * 0.3));
		} else {
			combatLevel = (int) (((getLevelForXP(playerXP[1])) * 0.5)
					+ ((getLevelForXP(playerXP[3])) * 0.4)
					+ ((getLevelForXP(playerXP[5])) * 0.35)
					+ ((getLevelForXP(playerXP[0])) * 0.3) + ((getLevelForXP(playerXP[2])) * 0.325));
		}
	...
	public void appendPlayerUpdateBlock(stream str) {
		...
		// crucial
		if (mask400update)
			appendMask400Update(str); // Xerozcheez: Very interesting update
										// mask!
		if (mask100update)
			appendMask100Update(str); // Xerozcheez: Graphics on player update
										// mask
		if (animationRequest != -1)
		...
		// if(update1Required) appendEmotionUpdate(str); // tested, thought it
		// was animation, and i was right! whitefang got it working for me :D -
		// xerozcheez
		// if(mask1update) appendMask1Update(str); // a strange update mask,
		// only one var (_) - xerozcheez
		// if(mask100update) appendMask100Update(str); // very interesting
		// update mask! - xerozcheez
	...
	public void clearUpdateFlags() { // Xerozcheez: ORDER IS CRUCIAL HERE TOO
										// (although it's different order I
										// think LOL) :|
			...
			for (i = 0; i < playerFollow.length; i++) {
				...
				if (!found) {
					...
					if (dir != -1 && (dir & 1) != 0) {
						...
						dir = misc.direction(wayPointX1, wayPointY1,
	...
	protected void appendHitUpdate2(stream str) {
		try {
			...
			if (hitDiff > 0 && newhptype == false && poisondmg == false) {
				...
				str.writeByteA(2); // 0: red hitting - 1: blue hitting 2: poison
									// 3: orange
			...
			str.writeByte(getLevelForXP(playerXP[playerHitpoints])); // Their
																		// max
																		// hp,
																		// for
																		// HP
	...
	protected void appendHitUpdate(stream str) {
		try {
			...
			str.writeByteC(getLevelForXP(playerXP[playerHitpoints])); // Their
																		// max
																		// hp,
																		// for
																		// HP
	...
	public void appendEmotionUpdate(stream str) {
		str.writeWordBigEndian((playerEmotionReq == -1) ? 65535
				: playerEmotionReq);
	...
	public void appendMask400Update(stream str) { // Xerozcheez: Something to do
													// with direction
	...
	public void appendString4(stream str) { // Xerozcheez: Interesting mask,
